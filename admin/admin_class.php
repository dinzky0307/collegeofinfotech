...
class Action
{
    ...
    function updateclass($id)
    {
        ...
        $q = "UPDATE class SET course=?, year=?, section=?, sem=?, subject=?, description=?, teacher=?, SY=? WHERE id=?";
        $stmt = $this->db->prepare($q);
        $stmt->execute([$course, $year, $section, $sem, $subject, $description, $teacher, $sy, $id]);

        // Check if the update was successful
        if ($stmt->rowCount() > 0) {
            // The update was successful
            // Redirect to class.php with a success message
    ...
    function updateteacher($id)
    {
        ...
        if ($stmt->rowCount() > 0) {
            // The update was successful
            // Redirect to teacherlist.php with a success message
